fn reply(message: &str) -> &str {
    ...
    // treat empty string
    if msg.is_empty() { return "Fine. Be that way!"; }

    // Reverse iter, avoid to consume all the iteration for only treat one char
    let mut iter = msg.chars().rev();
    // Verify if the last char is question
    let is_question = iter.next() == Some('?');
    // Continue with the remainded char, collect only alpha
    let alpha = iter.filter(|letter| letter.is_ascii_alphabetic()).collect::<String>();
    // because if iter is empty after filter operation and return true by default, chained is impossible, tested the length after
    let is_all_alpha = alpha.len() > 0 && alpha.chars().all(|letter| letter.is_uppercase());
    // treat all others cases
    match (is_all_alpha, is_question) {
        (true, true) => return "Calm down, I know what I'm doing!",
        (true, false) => return "Whoa, chill out!",
