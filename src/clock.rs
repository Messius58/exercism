...
impl Clock {
    ...
    pub fn add_minutes(&self, minutes: i32) -> Self {
        ...
        clock.convert(self.hour as i32, self.minutes as i32 + minutes);
        clock
    }

    fn convert(&mut self, hours: i32, minutes: i32) {
        let tot = hours * (Self::MINUTES_DURATION as i32) + minutes;
        let tot_u: u32 = if tot.is_negative() {
            let tmp = (-tot) as u32;
            tmp.next_multiple_of(Self::HOUR_DURATION * Self::MINUTES_DURATION) - tmp
        } else {
            tot as u32
        };
        self.hour = (tot_u / Self::MINUTES_DURATION % Self::HOUR_DURATION) as u8;
        self.minutes = (tot_u % Self::MINUTES_DURATION) as u8;
    }
}
