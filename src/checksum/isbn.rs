...
pub fn is_valid_isbn(isbn: &str) -> bool {
    ...
    isbn.chars()
        .rev()
        .filter(|c| *c != '-')
        // Utilise 2 accumulateurs pour répétitivement ajouter le chiffre à partir de sa position (éviter la multiplication)
        // cf https://en.wikipedia.org/wiki/ISBN#ISBN-10_check_digit_calculation
        .try_fold((0, 0, 0), |(sum, sum_prec, count), val| {
            let rslt = if val == 'X' {
                            if count == 0 { Some(10) } else { None }
                        }
                        else { val.to_digit(10) };
            rslt.map(|num| sum + num)
                .map(|num| (num, sum_prec + num, count + 1))
        }).map_or(false, |(_, sum_prec, count)| sum_prec % 11 == 0 && count == 10)
}
